...
st.header("Antyodaya Dashboard")

# Base Map
m = folium.Map(location=[17.620428, 79.237383],zoom_start=7, tiles='cartodbpositron')

# Reading state Data
df = pd.read_csv("dist_file_tel_ma.csv")
geoData = open('dist_file.json', 'r')

# Reading ac data
geo_ac_data = gpd.read_file("ac_file.json")
districts = list(geo_ac_data['District'].unique())
districts.insert(0, "All")
ac_df = pd.read_csv('acagg.csv')

# Reading gc data
geo_gc_data = gpd.read_file("gp_file_centroid.json")


# Creating dropdowns
category = st.sidebar.selectbox("Select a Category", list(df.columns[24:]))
...
if district == "All":
    ...
    choropleth = folium.Choropleth(
        ...
        name='choropleth',
        data=df,
        columns=['Dist_Nm', str(category)],
        key_on='feature.properties.Dist_Nm',
        fill_color='PuRd_r',
        fill_opacity=0.7,
        line_opacity=1,
